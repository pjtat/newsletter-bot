...
class MarkdownGenerator:
    ...
    def _build_markdown(self, articles: List[Article], profile: Dict, date_str: str) -> str:
        ...
        if articles:
            earliest = min(article.published for article in articles)
            latest = max(article.published for article in articles)
            earliest_str = earliest.strftime('%B %d')
            latest_str = latest.strftime('%B %d')
            date_range = f"*Generated {date_str} (articles from {earliest_str} - {latest_str})*\n"
        else:
            date_range = f"*Generated {date_str}*\n"
